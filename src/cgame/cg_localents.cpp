...
void CG_AddFragment( localEntity_t *le ) {
	...
	if ( le->onFireStart && ( le->onFireStart < cg.time && le->onFireEnd > cg.time ) ) {
		...
		S_AddLoopingSound( -1, le->refEntity.origin, vec3_origin, 1250, cgs.media.flameCrackSound, (int)( 5.0 * flameAlpha ) );
	}
...
//----(SA)	end
	...
	if ( hasFlame ) {
		...
		if ( VectorLength( flameDir ) == 0 ) {
			...
			S_AddLoopingSound( 0, newOrigin, vec3_origin, 1250, cgs.media.flameSound, (int)( 0.3 * 255.0 * flameAlpha ) );
		} else {
			VectorNormalize( flameDir );
			// play a flame blow sound when moving
			S_AddLoopingSound( 0, newOrigin, vec3_origin, 1250, cgs.media.flameBlowSound, (int)( 0.3 * 255.0 * flameAlpha ) );
		}
	}
...
void CG_AddClientCritter( localEntity_t *le ) {
	...
	if ( le->loopingSound ) {
		if ( cg.time > le->refEntity.fadeStartTime ) {
			S_AddLoopingSound( 0, le->refEntity.origin, vec3_origin, 1250, le->loopingSound, 255 - (int)( 255.0 * (float)( cg.time - le->refEntity.fadeStartTime ) / (float)( le->refEntity.fadeEndTime - le->refEntity.fadeStartTime ) ) );
		} else {
			S_AddLoopingSound( 0, le->refEntity.origin, vec3_origin, 1250, le->loopingSound, 255 - (int)( 255.0 * ( 1.0 - alpha ) ) );
		}
	}
/*
	if (le->leType == LE_HELGA_SPIRIT) {
