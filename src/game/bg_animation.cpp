	...
	while ( 1 ) {
		...
		if ( !Q_stricmp( token, "footsteps" ) ) {
			...
			if ( !Q_stricmp( token, "default" ) || !Q_stricmp( token, "normal" ) ) {
                ...
                return false; // linter - BG_AnimParseError does not return
			}
			continue;
...
void BG_ParseConditionBits( const char **text_pp, animStringItem_t *stringTable, int condIndex, int result[2] ) {
	...
	while ( !endFlag ) {
		...
		const char *token = COM_ParseExt( text_pp, false );
		if ( !token || !token[0] ) {
		...
		if ( !endFlag && Q_stricmp( token, "AND" ) && Q_stricmp( token, "MINUS" ) ) { // must be a index
			...
			if ( token[strlen( token ) - 1] == ',' ) {
				endFlag = true;
				// TODO: FIXME
				char* wtoken = (char*)(token + strlen( token ) - 1);
				*wtoken = '\0';
			}
...
=================
*/
bool BG_ParseConditions( const char **text_pp, animScriptItem_t *scriptItem ) {
...
void BG_ParseCommands( const char **input, animScriptItem_t *scriptItem, animModelInfo_t *modelInfo, animScriptData_t *scriptData ) {
	...
	while ( 1 ) {
		...
		// parse the body part
		const char* token = COM_ParseExt( input, ( partIndex < 1 ) );
		if ( !token || !token[0] ) {
			break;
		}
		if ( !Q_stricmp( token, "}" ) ) {
