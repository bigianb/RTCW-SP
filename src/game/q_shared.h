...
#define SNAPFLAG_RATE_DELAYED   1	// not used because local connection only
...
// PlayerState is a full superset of EntityState as it is used by players,
// so if a PlayerState is transmitted, the EntityState can be fully derived
// from it.
//
...
public:
	...
	int eFlags;                 // copied to EntityState->eFlags

	int eventSequence;          // pmove generated events
	int events[MAX_EVENTS];
	int eventParms[MAX_EVENTS];
	int oldEventSequence;           // so we can see which events have been added since we last converted to EntityState
	...
	// weapon info
	int weapon;                 // copied to EntityState->weapon
	int weaponstate;
...
// UserCmd is sent to the server each client frame
class UserCmd {
public:
	int serverTime;
	uint8_t buttons;
...
typedef enum {
	...
	CA_CONNECTED,       // NetChannel established, getting gamestate
