...
// it would fail.
static_assert( sizeof( unsigned int ) == 4, "This code assumes that unsigned int is 32 bits." );
...
========================
*/
inline bool IEEE_FLT_IS_NAN( float x )
{
	return x != x;
}
...
========================
*/
inline bool IEEE_FLT_IS_INF( float x )
{
	return x == x && x * 0 != x * 0;
}
...
========================
*/
inline bool IEEE_FLT_IS_INF_NAN( float x )
{
	return x * 0 != x * 0;
}
...
========================
*/
inline bool IEEE_FLT_IS_IND( float x )
{
...
========================
*/
inline bool IEEE_FLT_IS_DENORMAL( float x )
{
...
================================================================================================
*/
template<class T> inline int	MaxIndex( T x, T y )
{
	return ( x > y ) ? 0 : 1;
}
template<class T> inline int	MinIndex( T x, T y )
{
	return ( x < y ) ? 0 : 1;
}
...
template<class T> inline int	Max3Index( T x, T y, T z )
{
	return ( x > y ) ? ( ( x > z ) ? 0 : 2 ) : ( ( y > z ) ? 1 : 2 );
}
template<class T> inline int	Min3Index( T x, T y, T z )
{
	return ( x < y ) ? ( ( x < z ) ? 0 : 2 ) : ( ( y < z ) ? 1 : 2 );
}
...
================================================================================================
*/
template<class T> inline T	Sign( T f )
{
...
public:
	...
	static uint8_t					Ftob( float f );			// float to uint8_t conversion, the result is clamped to the range [0-255]
...
inline uint8_t CLAMP_BYTE( int x )
{
...
========================
*/
inline float idMath::Frac( float f )
{
	return f - floorf( f );
}
...
========================
*/
inline uint8_t idMath::Ftob( float f )
{
	// If a converted result is negative the value (0) is returned and if the
	// converted result is larger than the maximum uint8_t the value (255) is returned.

#if defined(USE_INTRINSICS_SSE)
	__m128 x = _mm_load_ss( &f );
	x = _mm_max_ss( x, SIMD_SP_zero );
	x = _mm_min_ss( x, SIMD_SP_255 );
	return static_cast<uint8_t>( _mm_cvttss_si32( x ) );
#else
