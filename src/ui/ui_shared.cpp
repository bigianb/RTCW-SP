...
const char *String_Alloc( const char *p ) {
	...
	const long hash = hashForString(p);
...
int Menu_ItemsMatchingGroup( menuDef_t *menu, const char *name ) {
	...
	const char* pdest = strstr( name, "*" ); // allow wildcard strings (ex.  "hide nb_*" would translate to "hide nb_pg1; hide nb_extra" etc)
	if ( pdest ) {
		wildcard = pdest - name;
	}
...
void Script_Show( itemDef_t *item, const char **args ) {
	const char *name;
	if ( String_Parse( args, &name ) ) {
		Menu_ShowItemByName( static_cast<menuDef_t*>(item->parent), name, true );
	}
}
...
void Script_Hide( itemDef_t *item, const char **args ) {
	const char *name;
	if ( String_Parse( args, &name ) ) {
		Menu_ShowItemByName( static_cast<menuDef_t*>(item->parent), name, false );
	}
}
...
void Script_FadeIn( itemDef_t *item, const char **args ) {
	const char *name;
	if ( String_Parse( args, &name ) ) {
		Menu_FadeItemByName( static_cast<menuDef_t*>(item->parent), name, false );
	}
}
...
void Script_FadeOut( itemDef_t *item, const char **args ) {
	const char *name;
	if ( String_Parse( args, &name ) ) {
		Menu_FadeItemByName( static_cast<menuDef_t*>(item->parent), name, true );
	}
}
...
void Script_Clipboard( itemDef_t *item, const char **args ) {
	...
	Menu_ShowItemByName( static_cast<menuDef_t*>(item->parent), curscript, true );
}
...
void Script_NotebookShowpage( itemDef_t *item, const char **args ) {
	...
	int pages = Cvar_VariableValue("cg_notebookpages");

	if ( Int_Parse( args, &inc ) ) {

		int curpage = Cvar_VariableValue("ui_notebookCurrentPage");
		...
		// hide all the pages
		Menu_ShowItemByName( static_cast<menuDef_t*>(item->parent), "cover", false );
		for (int i = 1; i <= NOTEBOOK_MAX_PAGES; i++ ) {
			Menu_ShowItemByName( static_cast<menuDef_t*>(item->parent), va( "page%d", i ), false );
		}

		// show the visible one

		if ( curpage ) {
			Menu_ShowItemByName( static_cast<menuDef_t*>(item->parent), va( "page%d", curpage ), true );
		} else {
...
bool Item_SetFocus( itemDef_t *item, float x, float y ) {
	...
	parent = static_cast<menuDef_t*>(item->parent);
...
float Item_Slider_ThumbPosition( itemDef_t *item ) {
	...
	float value = Cvar_VariableValue(item->cvar);
...
int Item_Slider_OverSlider( itemDef_t *item, float x, float y ) {
	...
	r.x = Item_Slider_ThumbPosition( item ) - ( SLIDER_THUMB_WIDTH / 2.0f );
...
extern bool UI_OwnerDrawHandleKey( int ownerDraw, int flags, float *special, int key );
...
static void Scroll_Slider_ThumbFunc( void *p ) {
	...
	const editFieldDef_t *editDef = static_cast<editFieldDef_t*>(si->item->typeData);
	...
	while ( p ) {
		...
		if ( ( newLine && textWidth > item->window.rect.w ) || *p == '\n' || *p == '\0' ) {
			if ( len ) {
				...
				//
				buff[newLine] = '\0';
                Text_Paint( item->textRect.x, item->textRect.y, item->font, item->textscale, color, buff, 0, 0, item->textStyle );
			}
			if ( *p == '\0' ) {
				break;
			}
...
void Item_Text_Wrapped_Paint( itemDef_t *item ) {
	...
	while ( p && *p ) {
		...
		buff[p - start] = '\0';
        Text_Paint( x, y, item->font, item->textscale, color, buff, 0, 0, item->textStyle );
		y += height + 5;
		start += p - start + 1;
		p = strchr( p + 1, '\r' );
	}
    Text_Paint( x, y, item->font, item->textscale, color, start, 0, 0, item->textStyle );
}
...
//----(SA)	end
    ...
    Text_Paint( item->textRect.x, item->textRect.y, item->font, item->textscale, color, textPtr, 0, 0, item->textStyle );
}
...
void Item_TextField_Paint( itemDef_t *item ) {
	...
	if ( item->window.flags & WINDOW_HASFOCUS && g_editingField ) {
		const char cursor = Key_GetOverstrikeMode() ? '_' : '|';
		Text_PaintWithCursor( item->textRect.x + item->textRect.w + offset, item->textRect.y, item->font, item->textscale, newColor, buff + editPtr->paintOffset, item->cursorPos - editPtr->paintOffset, cursor, editPtr->maxPaintChars, item->textStyle );
	} else {
        Text_Paint( item->textRect.x + item->textRect.w + offset, item->textRect.y, item->font, item->textscale, newColor, buff + editPtr->paintOffset, 0, editPtr->maxPaintChars, item->textStyle );
	}
...
void Item_YesNo_Paint( itemDef_t *item ) {
	vec4_t newColor, lowLight;
	auto *parent = static_cast<menuDef_t*>(item->parent);
	auto yes_str = "yes", no_str = "no";

	const float value = (item->cvar) ? Cvar_VariableValue(item->cvar) : 0;
	...
	if ( item->text ) {
		Item_Text_Paint( item );
        Text_Paint( item->textRect.x + item->textRect.w + 8, item->textRect.y, item->font, item->textscale, newColor, ( value != 0 ) ? yes_str : no_str, 0, 0, item->textStyle );
	} else {
        Text_Paint( item->textRect.x, item->textRect.y, item->font, item->textscale, newColor, ( value != 0 ) ? yes_str : no_str, 0, 0, item->textStyle );
	}
}
...
void Item_Multi_Paint( itemDef_t *item ) {
	...
	if ( item->text ) {
		Item_Text_Paint( item );
        Text_Paint( item->textRect.x + item->textRect.w + 8, item->textRect.y, item->font, item->textscale, newColor, text, 0, 0, item->textStyle );
	} else {
        Text_Paint( item->textRect.x, item->textRect.y, item->font, item->textscale, newColor, text, 0, 0, item->textStyle );
	}
}
...
static constexpr int g_bindCount = sizeof( g_bindings ) / sizeof( bind_t );
...
void Controls_GetConfig( ) {
	...
	for (auto & g_binding : g_bindings)
	{
		Controls_GetKeyAssignment( g_binding.command, twokeys );

		g_binding.bind1 = twokeys[0];
		g_binding.bind2 = twokeys[1];
	}
}

void Controls_SetConfig( bool restart ) {
	// iterate each command, get its numeric binding
	for (const auto & g_binding : g_bindings)
	{
		if ( g_binding.bind1 != -1 ) {
			Key_SetBinding( g_binding.bind1, g_binding.command );

			if ( g_binding.bind2 != -1 ) {
                Key_SetBinding( g_binding.bind2, g_binding.command );
			}
		}
	}
...
void Controls_SetDefaults( ) {
	...
	for (auto & g_binding : g_bindings)
	{
		g_binding.bind1 = g_binding.defaultbind1;
		g_binding.bind2 = g_binding.defaultbind2;
	}
}
...
void BindingFromName( const char *cvar ) {
	...
	for (const auto & g_binding : g_bindings)
	{
		if ( Q_stricmp( cvar, g_binding.command ) == 0 ) {
			const int b1 = g_binding.bind1;
			if ( b1 == -1 ) {
				break;
			}
...
void Item_Slider_Paint( itemDef_t *item ) {
	...
	float value = (item->cvar) ? Cvar_VariableValue(item->cvar) : 0;
...
void Item_Bind_Paint( itemDef_t *item ) {
	...
	if ( item->text ) {
        ...
        Text_Paint( item->textRect.x + item->textRect.w + 8, item->textRect.y, item->font, item->textscale, newColor, g_nameBind1, 0, maxChars, item->textStyle );
	} else {
        Text_Paint( item->textRect.x, item->textRect.y, item->font, item->textscale, newColor, ( value != 0 ) ? "FIXME" : "FIXME", 0, maxChars, item->textStyle );
	}
}
...
bool Item_Bind_HandleKey( itemDef_t *item, int key, bool down ) {
	...
	if ( key != -1 )
	{
		for (auto & g_binding : g_bindings)
		{
			...
			if ( g_binding.bind2 == key ) {
				g_binding.bind2 = -1;
			}

			if ( g_binding.bind1 == key ) {
				g_binding.bind1 = g_binding.bind2;
				g_binding.bind2 = -1;
			}
		}
	}
...
void Item_Model_Paint( itemDef_t *item ) {
	...
	const qhandle_t hModel = item->asset;
	...
	if ( modelPtr->backlerp > 1 ) {
		const int backLerpWhole = floor(modelPtr->backlerp);
...
void Item_OwnerDraw_Paint( itemDef_t *item ) {
	...
	if ( item->numColors > 0) {
		...
		const float f = CG_GetValue( item->window.ownerDraw, item->colorRangeType );
...
void Item_Paint( itemDef_t *item ) {
	vec4_t red;
	const menuDef_t *parent = static_cast<menuDef_t*>(item->parent);
	red[0] = red[3] = 1;
	red[1] = red[2] = 0;
...
void Menu_SetFeederSelection( menuDef_t *menu, int feeder, int index, const char *name ) {
	...
	if ( menu ) {
		for ( int i = 0; i < menu->itemCount; i++ ) {
			if ( menu->items[i]->special == feeder ) {
				if ( index == 0 ) {
					auto *listPtr = static_cast<listBoxDef_t*>(menu->items[i]->typeData);
					listPtr->cursorPos = 0;
					listPtr->startPos = 0;
				}
...
keywordHash_t itemParseKeywords[] = {
	...
	{"textsavegame", ItemParse_textsavegame, nullptr},  //----(SA)	added
	...
	{"textfont", ItemParse_textfont, nullptr},
...
bool MenuParse_onROQDone( itemDef_t *item, int handle ) {
	auto *menu = (menuDef_t*)item;
	return PC_Script_Parse( handle, &menu->onROQDone );
}
...
bool MenuParse_backcolor( itemDef_t *item, int handle ) {
	...
	for (float & i : menu->window.backColor) {
		float f;
		if ( !PC_Float_Parse( handle, &f ) ) {
			return false;
		}
...
bool MenuParse_forecolor( itemDef_t *item, int handle ) {
	...
	for (float & i : menu->window.foreColor) {
		float f;
		if ( !PC_Float_Parse( handle, &f ) ) {
			return false;
		}
...
bool MenuParse_bordercolor( itemDef_t *item, int handle ) {
	...
	for (float & i : menu->window.borderColor) {
		float f;
		if ( !PC_Float_Parse( handle, &f ) ) {
			return false;
		}
...
bool MenuParse_cinematic( itemDef_t *item, int handle ) {
	...
	return PC_String_Parse( handle, &menu->window.cinematicName );
}
...
bool MenuParse_soundLoop( itemDef_t *item, int handle ) {
	...
	return PC_String_Parse( handle, &menu->soundName );
}
...
bool MenuParse_itemDef( itemDef_t *item, int handle ) {
	...
	if ( menu->itemCount < MAX_MENUITEMS ) {
        ...
        const itemDef_t* childDef = menu->items[menu->itemCount];
...
bool MenuParse_execKey( itemDef_t *item, int handle ) {
	...
	short int keyindex = static_cast<unsigned char>(keyname);
...
keywordHash_t menuParseKeywords[] = {
	...
	{"onROQDone", MenuParse_onROQDone, nullptr},    //----(SA)	added
...
void Menu_New( int handle, bool isHud ) {
...
void Menu_PaintAll() {
	...
	for ( i = 0; i < hudMenuCount; i++ ) {
		Menu_Paint( &HudMenus[i], false );
	}

	if ( debugMode ) {
		vec4_t v = {1, 1, 1, 1};
        Text_Paint( 5, 25, 0, .5, v, va( "fps: %f", DC->FPS ), 0, 0, 0 );
	}
}

void Menu_Reset(bool isHud) {
	if (isHud){
		hudMenuCount = 0;
	} else {
		menuCount = 0;
...
void Display_HandleKey( int key, bool down, int x, int y ) {
	auto *menu = static_cast<menuDef_t*>(Display_CaptureItem(x, y));
	if ( menu == nullptr ) {
		menu = Menu_GetFocused();
	}
	if ( menu ) {
